...
export async function install(): Promise<void> {
  ...
  const version = core.getInput('version') || (await getLatestVersion(auth))
  ...
  if (!cliPath) {
    const downloadPath = await downloadTool(packageUrl, 'gh_tar', auth)
...
export async function getLatestVersion(auth?: string): Promise<string> {
